    ...
    <div className="min-h-screen flex items-center justify-center bg-[var(--bg-page)] text-[var(--text-primary)]">
      <div className="w-full max-w-md p-8 glass-panel-strong rounded-3xl shadow-2xl">
        <div className="text-center mb-10">
          <div className="w-16 h-16 bg-[var(--accent)] rounded-2xl mx-auto mb-4 flex items-center justify-center shadow-lg shadow-[var(--accent)]/20">
            <span className="text-2xl font-bold">A</span>
          </div>
          <h1 className="text-2xl font-light tracking-tight">管理员登录</h1>
          <p className="text-[var(--text-muted)] text-sm mt-2">公开浏览默认可用，登录后才会显示编辑与管理功能</p>
        </div>

        <form onSubmit={handleSubmit} className="space-y-6" aria-busy={isSubmitting}>
          <div>
            <label className="block text-xs font-medium text-[var(--text-muted)] mb-2 uppercase tracking-widest">用户名</label>
            <input
              type="text"
              value={username}
              onChange={(e) => setUsername(e.target.value)}
              disabled={isSubmitting}
              className="surface-input w-full rounded-xl px-4 py-3 focus:outline-none focus-theme transition-all font-mono"
              required
            />
          </div>
          <div>
            <label className="block text-xs font-medium text-[var(--text-muted)] mb-2 uppercase tracking-widest">密码</label>
            <input
              type="password"
              value={password}
          ...
          {isSubmitting && (
            <div className="rounded-xl success-soft px-4 py-3 text-center text-xs">
              正在验证身份并进入页面，请稍候...
            </div>
          )}
          ...
          <div className="text-center mt-6">
            ...
            <p className="mt-3 text-[var(--text-muted)] text-sm">
              本地第一次启动？
              <Link href="/setup" className="ml-1 text-[var(--color-completed)] hover:text-[var(--accent-light)]">
                打开初始化向导 /setup
              </Link>
            </p>
          </div>
        ...
        <div className="mt-8 pt-8 border-t border-[var(--border)] text-center text-[10px] text-[var(--text-muted)] font-mono tracking-widest">
