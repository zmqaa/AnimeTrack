          ...
          {recentWatchItems.length > 0 ? recentWatchItems.map((item) => (
            ...
            >
              <div className="surface-card-muted w-10 h-10 rounded-lg overflow-hidden flex-shrink-0 group-hover/item:border-[var(--color-watching)]/30 transition-colors shadow-lg relative">
                {(item.thumbnailCoverUrl || item.displayCoverUrl) ? (
                  <Image key={item.thumbnailCoverUrl || item.displayCoverUrl} src={item.thumbnailCoverUrl || item.displayCoverUrl || ''} fill unoptimized sizes="40px" className="object-cover transition-transform group-hover/item:scale-110" alt="" />
                ) : (
