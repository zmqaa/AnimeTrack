...
8. 只有用户明确说出季度，或直接说出了该续作完整官方标题（包括用于区分续作的副标题、符号或英文）时，才能返回 season>1 的续作。用户只说系列基础标题时，绝对不能自行推断为第二季或更后面的续作。
9. 只有用户明确提到的信息才填写；不知道就用 null、空字符串或空数组，不要补全设定。
10. "以前、之前、小时候、很久前、早就"这类表述，isHistorical=true；没给具体日期时 watchedAt 留空。
